...
export const getSymbolKey = ({ rawSymbol }: { rawSymbol: RawSymbol }) => {
	if (rawSymbol.multiplier !== undefined) {
		const multKey = `${rawSymbol.name}_${rawSymbol.multiplier}` as keyof typeof SYMBOL_INFO_MAP;
		if (multKey in SYMBOL_INFO_MAP) return multKey;
	}
	if (rawSymbol.name in SYMBOL_INFO_MAP) {
		return rawSymbol.name as keyof typeof SYMBOL_INFO_MAP;
	}
	return 'H1' as keyof typeof SYMBOL_INFO_MAP;
};
